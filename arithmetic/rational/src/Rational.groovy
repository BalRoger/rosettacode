class Rational implements Comparable {
    ...
    Rational(BigDecimal decimal) {
        this(
            //                    INTEGER VALUE                                    NON-INTEGER VALUE
    ...
    Rational mod(BigInteger n) { new Rational(num % n, denom) }

    BigInteger intdiv(BigInteger n) { num.intdiv(denom*n) }

    Rational negative() { [-num, denom] }

    Rational getAbs() { [num.abs(), denom] }
    Rational abs() { this.abs }
    static Rational abs(Rational r) { r.abs }

    Rational getReciprocal() { new Rational(denom, num) }
    Rational reciprocal() { this.reciprocal }
    static Rational reciprocal(Rational r) { r.reciprocal }
