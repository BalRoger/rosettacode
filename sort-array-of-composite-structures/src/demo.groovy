class Holiday {
    def date
    def name
    Holiday(dateStr, name) { this.name = name; this.date = Date.parse(FORMAT, dateStr) }
    String toString() { "${date.format FORMAT}: ${name}" }
...
holidays.sort(true) { x, y -> x.date <=> y.date }
